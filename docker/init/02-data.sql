...
INSERT INTO motivation_messages (content) VALUES 
('천천히 가도 괜찮아요. 한 걸음 내디었다는 게 중요해요.'),
('당신의 노력은 언젠가 가장 빛나는 날을 만들어줄 거예요.'),
('지금은 추운 겨울 같아도, 곧 당신의 봄이 찾아올 거예요.'),
('포기하지 않는 것, 그것만으로도 이미 대단한 사람이에요.'),
('세상 누구보다 스스로를 믿어주세요. 당신의 길은 분명히 있어요.'),
('누구보다 열심히 살아온 당신, 분명 좋은 소식이 찾아올 거예요.'),
('한 걸음 내디뎠다는 것, 그 자체로 충분히 의미 있어요.'),
( '지금 당신이 걷고 있는 길이 새로운 봄을 향해 가고 있어요.'),
('취업 준비의 시간이 때론 길게 느껴질 수 있지만, 그 시간이 당신을 더 단단하게 만들고 있어요. 새봄이 찾아올 거예요.'),
('당신의 노력은 절대 헛되지 않아요. 눈에 보이지 않아도 분명히 쌓이고 있답니다.'),
('작은 진전도 소중한 성취예요. 오늘 하루도 수고했습니다.'),
('새로운 시작은 언제나 용기가 필요해요. 우린 해냈어요!'),
('겨울이 지나면 반드시 봄이 오듯, 당신의 노력 끝에는 분명 좋은 결실이 기다리고 있어요.'),
('당신만의 속도로 걸어가도 괜찮아요. 비교하지 말고 자신의 페이스를 지켜나가세요. 새봄은 모두에게 다른 시간에 찾아옵니다.'),
('새봄이처럼, 당신의 하루도 따뜻해지길 바라요.');
